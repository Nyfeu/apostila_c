...
O \code{gcc} executa essas quatro etapas automaticamente, mas é possível observá-las isoladamente:
...
A função \code{srand} define a semente do gerador; usar \code{time(NULL)} (o tempo atual em segundos) garante uma semente diferente a cada execução. A função \code{rand()} retorna um inteiro pseudoaleatório; o operador \code{\%} (resto da divisão) é usado para restringir o resultado a um intervalo específico — no exemplo, de 1 a 6, como em um dado de tabuleiro.
...
\begin{esp32box}
O \ESP{} não tem, por padrão, um relógio de tempo real com precisão de segundos ao ligar (a menos que sincronizado via Wi-Fi/NTP), então usar \code{time(NULL)} como semente nem sempre é útil logo na inicialização. No framework Arduino, o par \code{randomSeed()}/\code{random()} substitui \code{srand}/\code{rand}: um truque comum é semear o gerador lendo um pino analógico sem nada conectado a ele (\code{randomSeed(analogRead(A0))}), aproveitando o ruído elétrico natural presente nesse pino como fonte de aleatoriedade. Por baixo dos panos, o núcleo Arduino para o \ESP{} ainda pode recorrer a um gerador de números aleatórios embutido no próprio chip — mas, na prática do dia a dia, \code{randomSeed}/\code{random} já resolvem bem a maioria dos casos.
\end{esp32box}
