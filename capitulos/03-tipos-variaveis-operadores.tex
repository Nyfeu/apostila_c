...
\code{\_Bool} / \code{bool}\textsuperscript{†} & valor lógico (verdadeiro/falso) & 1 byte \\
\bottomrule
\end{tabular}
...
Quando uma expressão mistura tipos diferentes, C converte automaticamente os valores para um tipo comum antes de operar — a chamada \textbf{conversão implícita}. Isso já apareceu antes: \code{7.0 / 2} funciona porque o \code{2} (inteiro) é implicitamente convertido para \code{2.0} (ponto flutuante) antes da divisão.
