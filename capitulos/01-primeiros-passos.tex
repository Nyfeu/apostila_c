...
A diferença mais importante a guardar por enquanto é estrutural. Você viu que o nosso programa C tradicional usa uma função \code{main} que roda uma vez, exibe a mensagem na tela, e termina devolvendo \code{0} ao sistema operacional. 

Um microcontrolador funciona de forma diferente. Não faria sentido um programa finalizar sua execução — se ele acabar, o que o hardware faria depois? Por isso, a programação para o ESP32 é baseada em um \textbf{ciclo contínuo}. Em vez de uma função que termina, o código de um firmware é desenhado para rodar infinitamente, repetindo instruções e reagindo ao mundo exterior enquanto a placa estiver ligada. 

Quando chegarmos na Parte II e formos programar o ESP32 usando o framework Arduino, essa estrutura aparecerá de forma bem explícita: em vez de usarmos o \code{main}, dividiremos nosso código em duas partes: uma função chamada \code{setup()} para configurações iniciais, e uma função chamada \code{loop()}, que o próprio sistema se encarrega de repetir para sempre.
